...
estadisticasModel.get=function(id,f1,f2,callback){
	connection.query('SELECT  AVG(estudio.Salinidad) AS salinidad, AVG(estudio.Temperatura) AS temperatura, AVG(estudio.Conductividad) AS conductividad, AVG(estudio.Ph) AS ph, arribo.ID_buque, buque.Nombre_buque , SUM(arribo.Volumen_total) AS Voltotal FROM arribo, puerto, estudio, buque WHERE arribo.ID_puerto=puerto.ID_puerto AND estudio.ID_arribo=arribo.ID_arribo AND arribo.ID_buque=buque.ID_buque AND arribo.Fecha_arribo BETWEEN "'+f1+'" AND "'+f2+'" AND buque.ID_buque='+id, function(err, rows, fields) {
	  if (err) throw err;
	  callback(rows);
	});
}
...
estadisticasModel.show=function(id,f1,f2,callback){
	connection.query('SELECT AVG(estudio.Salinidad) AS salinidad, AVG(estudio.Temperatura) AS temperatura, AVG(estudio.Conductividad) AS conductividad, AVG(estudio.Ph) AS ph, arribo.ID_puerto,puerto.Nombre_puerto, SUM(arribo.Volumen_total) AS Voltotal FROM arribo, puerto, estudio, buque WHERE arribo.ID_puerto=puerto.ID_puerto AND arribo.ID_buque=buque.ID_buque AND estudio.ID_arribo=arribo.ID_arribo AND arribo.Fecha_arribo BETWEEN "'+f1+'" AND "'+f2+'" AND puerto.ID_puerto='+id, function(err, rows, fields) {
	  if (err) throw err;
	  callback(rows);
	});
}
